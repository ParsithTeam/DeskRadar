...
│       └── ticket_embeddings.json       # persistent ticket-vector cache
├── scripts/
...
├── docs/
│   ├── backend_integration_contract.md  # Backend handoff and persistence rules
...
> `matched_ticket_ids`, `avg_similarity_score`, `is_duplicate`, `duplicate_incident_id`). The Backend lifts the
...
The standalone AI Infrastructure component is verified for the default
Python-cosine mode with Qdrant disabled: 22 automated tests pass, the real-model
separation gap is `0.5914`, and retrieval category accuracy is `0.9333`. The
remaining project-level work is the documented Backend/Analyzer integration.
